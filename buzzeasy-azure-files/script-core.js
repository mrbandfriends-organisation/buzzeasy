(function($){
	...
	// Prepend 'by' to blog author

	if($('.gdlr-core-blog-info-author').length) {

		var blogAuthor = $('.gdlr-core-blog-info-author a');
		blogAuthor.prepend('by ');
	}

	// Remove year from Blog Dates

	if($('.gdlr-core-blog-info-date').length) {


		var blogDate = $('.gdlr-core-blog-info-date a').html();

		var newblogDate = blogDate.substr(0, blogDate.length-4);

		$('.gdlr-core-blog-info-date a').html(newblogDate)
	}
