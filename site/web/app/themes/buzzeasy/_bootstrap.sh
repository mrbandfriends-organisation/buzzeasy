...
# c. check existence
if [ -d $INSTALL_DIR ] && [ -d $INSTALL_DIR/trellis ]; then
    printf '%b' "\e[1;91mWarning\e[0m: there seems to be something already installed there.\nExiting\n\n"
    exit 2
fi
...
printf '%b' "done\n"
printf '%b' "  - Clonings assets repo into Mr B WP theme"
cd $INSTALL_DIR/site/web/app/themes/$THEME_NAME && git clone --quiet --recursive git@mrbandfriends.git.beanstalkapp.com:/mrbandfriends/frontend-boilerplate-assets.git assets && rm -rf assets/.git > /dev/null || barf "um…?"
printf '%b' "done\n"
