 ...
 * @param  integer $word_limit the max number of words to return
...
/**
 * GET POST THUMBNAIL URL/ALT.
 *
 * Get url and alt attributes of a posts thumbnail image
 *
 * @param int $post_id of image you wish to get data for
 * @param string desired size of returned image
 */
 ...
 *
 * @param (mixed) (data) Pages slug
 * @param (string) (fallback) Fallback if variable doesn't exist, defaults to ''
 */
function check_for_empty($data = '', $fallback = '') {
...
function get_active_features() {
    ...
    foreach ($filterd_flags as $key => $value) {
        $new_mapped_array[$key] = true;
    }
...
function get_inactive_features() {
    // Filter through Feature Flags and return only true variables
    $filterd_flags = array_filter(FEATURE_FLAGS, function($feature){
        return !feature_active($feature);
    }, ARRAY_FILTER_USE_KEY);
